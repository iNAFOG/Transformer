...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) ->None:
        ...
        pe =  pe.unsquueze(0) #(1,seq_len,d_model)

        self.register_buffer('pe', pe) #register buffer to not be trained
...
def build_transformer(src_vocab_size : int, tgt_vocab_size :  int, src_seq_len: int, tgt_seq_len: int, d_model:int = 512, N:int = 6, h:int = 8, dropout : float =0.1, d_ff :int = 2048) ->Tranformer:
